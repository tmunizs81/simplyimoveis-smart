...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    // Prioridade: Groq (funciona em ambos ambientes) > Lovable AI (Cloud only)
    const GROQ_API_KEY = Deno.env.get("GROQ_API_KEY");
    const LOVABLE_API_KEY = Deno.env.get("LOVABLE_API_KEY");

    const useLovableAI = !GROQ_API_KEY && !!LOVABLE_API_KEY;
    const apiKey = GROQ_API_KEY || LOVABLE_API_KEY;
    if (!apiKey) throw new Error("Nenhuma API key configurada (GROQ_API_KEY ou LOVABLE_API_KEY)");
