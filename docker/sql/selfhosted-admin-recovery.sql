-- ============================================================
-- Simply Imóveis - Recovery/Bootstrap SQL (self-hosted VPS)
-- Idempotente: pode ser executado mais de uma vez
-- Pipeline: extensões → tipos → tabelas → funções → dados → RLS
-- ============================================================
...
-- has_role: usa o enum app_role (para uso nas RLS de tabelas public)
...
-- has_role_text: usa text (para uso nas RLS de storage.objects que não veem o enum)
...
BEGIN
  ...
  INSERT INTO public.property_code_sequences(prefix, last_number) VALUES (v_prefix, 0) ON CONFLICT (prefix) DO NOTHING;
...
-- update_property_short_code_on_status_change
...
-- ════════════════════════════════════════════════════════════
-- 5) Dados mínimos (seed)
-- ════════════════════════════════════════════════════════════
INSERT INTO public.property_code_sequences(prefix, last_number) VALUES ('A', 0), ('V', 0) ON CONFLICT (prefix) DO NOTHING;
...
BEGIN
  FOREACH tbl IN ARRAY ARRAY[
    'properties','property_media','property_code_sequences',
    'contact_submissions','scheduled_visits','leads','tenants',
    'rental_contracts','sales','financial_transactions',
    'property_inspections','inspection_media',
    'contract_documents','tenant_documents','sales_documents',
    'user_roles'
  ]
  LOOP
...
CREATE TRIGGER set_short_code BEFORE INSERT ON public.properties FOR EACH ROW EXECUTE FUNCTION public.generate_property_short_code();

DROP TRIGGER IF EXISTS update_short_code ON public.properties;
CREATE TRIGGER update_short_code BEFORE UPDATE ON public.properties FOR EACH ROW EXECUTE FUNCTION public.update_property_short_code_on_status_change();
...
CREATE TRIGGER update_property_inspections_updated_at BEFORE UPDATE ON public.property_inspections FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

-- ════════════════════════════════════════════════════════════
-- 8) Policies — properties
-- ════════════════════════════════════════════════════════════
...
CREATE POLICY "Anyone can view active properties" ON public.properties FOR SELECT TO public USING (active = true);
...
-- ════════════════════════════════════════════════════════════
-- 10) Policies — admin tables (FOR ALL)
-- ════════════════════════════════════════════════════════════
...
-- ════════════════════════════════════════════════════════════
-- 11) Policies — public/contact/visits
-- ════════════════════════════════════════════════════════════
