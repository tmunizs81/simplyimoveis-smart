...
MISSING=0
for f in .env docker-compose.yml sql/selfhosted-admin-recovery.sql sql/bootstrap-storage.sql \
         sync-functions.sh render-kong-config.sh bootstrap-db.sh ensure-storage-buckets.sh \
         sync-db-passwords.sh validate-install.sh; do
  if [ ! -f "$f" ]; then echo -e "   ${RED}❌ Ausente: $f${NC}"; MISSING=$((MISSING + 1)); fi
done
[ "$MISSING" -gt 0 ] && echo -e "${RED}❌ $MISSING arquivo(s) ausente(s).${NC}" && exit 1
